  ...
  return (
    <main className="animate-fade-in" style={{ paddingBottom: 100, minHeight: "100vh", backgroundColor: "var(--color-background)" }}>
      <div style={{ position: "absolute", top: 16, left: 16, zIndex: 10 }}>
        <button onClick={() => router.back()} style={{ backgroundColor: "rgba(255,255,255,0.85)", backdropFilter: "blur(4px)", border: "none", width: 40, height: 40, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--color-primary)", cursor: "pointer", boxShadow: "var(--shadow-sm)" }}>
      ...
      <div className="product-detail-layout">
        ...
        <div className="product-info-container" style={{ padding: "24px 20px", backgroundColor: "#fff", borderRadius: "24px 24px 0 0", marginTop: -24, position: "relative", boxShadow: "0 -4px 16px rgba(0,0,0,0.05)" }}>
          <span style={{ backgroundColor: "var(--color-background-alt)", color: "var(--color-primary)", padding: "4px 14px", borderRadius: 999, fontSize: 12, fontWeight: 600, display: "inline-block", marginBottom: 12 }}>
            {product.category}
          </span>

          <h1 className="font-serif" style={{ fontSize: 24, color: "var(--color-text-main)", marginBottom: 8, lineHeight: 1.2 }}>{product.title}</h1>
          <div style={{ fontSize: 24, color: "var(--color-primary)", fontWeight: 700, marginBottom: 24 }}>{fmt(product.price)}</div>
      ...
      <div style={{ position: "fixed", bottom: 0, width: "100%", maxWidth: 1200, backgroundColor: "#fff", borderTop: "1px solid var(--color-border)", padding: "16px 20px 24px", zIndex: 50, boxShadow: "0 -4px 24px rgba(139,34,82,0.06)" }}>
        <button className="btn-primary" onClick={() => { addToCart(product); router.push("/orcamento"); }} style={{ fontSize: 15 }}>
