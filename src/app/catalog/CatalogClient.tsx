...
function CatalogContent({ products }: { products: Product[] }) {
  ...
  return (
    <main className="animate-fade-in" style={{ paddingBottom: 90, minHeight: "100vh", backgroundColor: "var(--color-background)" }}>
      <header style={{ padding: "20px 20px 12px", backgroundColor: "#fff", position: "sticky", top: 0, zIndex: 10, borderBottom: "1px solid var(--color-border)" }}>
        ...
        <div style={{ position: "relative", width: "100%" }}>
          ...
          <input type="text" placeholder="Buscar por nome ou categoria..." value={search}
            onChange={(e) => { setSearch(e.target.value); setActiveFilter("Todos"); }}
            style={{ width: "100%", padding: "12px 16px 12px 44px", borderRadius: 999, border: "1px solid var(--color-border)", backgroundColor: "var(--color-background)", fontFamily: "inherit", fontSize: 13, outline: "none", color: "var(--color-text-main)" }}
          />
        </div>
        ...
        {filteredProducts.length > 0 ? filteredProducts.map(product => (
          ...
          >
            ...
            <div style={{ padding: "10px 12px 12px" }}>
              <span style={{ fontSize: 12, fontWeight: 500, color: "var(--color-text-main)", lineHeight: 1.3, display: "block", minHeight: 32 }}>{product.title}</span>
              <span style={{ fontSize: 14, color: "var(--color-primary)", fontWeight: 700 }}>{fmt(product.price)}</span>
            </div>
            <button onClick={(e) => { e.stopPropagation(); if (!isInCart(product.id)) addToCart(product); router.push("/orcamento"); }}
              style={{ position: "absolute", bottom: 56, right: 8, width: 44, height: 44, borderRadius: "50%", background: isInCart(product.id) ? "linear-gradient(135deg, #2b8a3e, #1a7a30)" : "linear-gradient(135deg, #C5668E, #8B2252)", color: "white", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "var(--shadow-md)", zIndex: 20, touchAction: "manipulation", WebkitTapHighlightColor: "transparent" }}
